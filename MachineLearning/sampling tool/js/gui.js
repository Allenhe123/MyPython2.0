...
function GUI_CLASS() {
	...
	this.autodetect_dimensions = function(){
		...
		if(save_resolution_cookie != ''){
			...
			console.log("screen cookie size, width", WIDTH, "height", HEIGHT);
		}
		else{
			WIDTH = page_w;
			HEIGHT = page_h;
			...
			for(var i = this.common_dimensions.length-1; i >= 0; i--){
				...
				auto_size = true;
				console.log("screen page size, width", page_w, "height", page_h);
				console.log("screen auto size, width", WIDTH, "height", HEIGHT);
				break;
			}
		...
		// 5x5 grid in a row
		this.grid_size[0] = Math.round(WIDTH/25);
		this.grid_size[1] = this.grid_size[0];
		console.log("grid size (x, y): (%d, %d)", this.grid_size[0], this.grid_size[1]);
	};
	...
	this.draw_background = function (canvas, W, H, gap, force) {
		...
		//canvas.fillStyle = '#8080ff';
		//canvas.font = '140px serif';
		//canvas.fillText("Dell EMC Hackathon", 140, 150);
	};
	...
	this.progress = function (percent) {
		var elem = document.getElementById("sample_bar");
		elem.style.width = percent + "%";
		elem.innerHTML = percent * 1 + '%';
	}
	*/

	this.draw_sample = function (pos, sample) {
		var dot_w = 2;
		var margin_w = 15;
		var left = pos * 2 * this.grid_size[0] + margin_w;
		var top = 10 * this.grid_size[1] + margin_w;
		var w = 2 * this.grid_size[0] - 2* margin_w;
		var h = 2 * this.grid_size[1] - 2* margin_w;	
		var color_rgb = HELPER.hex2rgb(COLOR);
		canvas_active().fillStyle = "rgba(" + color_rgb.r + ", " + color_rgb.g + ", " + color_rgb.b + ", " + ALPHA / 255 + ")";
		for (var i = 0; i < sample.length; i++) {
			var x = Math.round(left + sample[i][0] * w / 100);
			var y = Math.round(top + (100 - sample[i][1]) * h / 100);
			canvas_active().fillRect(x, y, dot_w, dot_w);
		}
	}
}
