...
function DRAW_TOOLS_CLASS() {
	...
	/**
	 * mouse point array to recording the moving track
	 */
	var points_track = [];
	var points_sample = [];
	var sample_pos = 0;
	...
	/**
	 * const used in sample
	 */
	var POINTS_SAMPLE_MAX = 80;
	var POINTS_SAMPLE_NUM = 8;
	var POINTS_TRACK_NUM_MIN = 8;
	...
	this.brush = function (type, mouse, event) {
		...
		if (brush_type == 'Brush') {
			if (type == 'click') {
				...
				// allow to undo
				undo_counter++;
				console.log("brush slected: x", mouse.x, ", y", mouse.y);
			}
			else if (type == 'drag' && mouse.last_x != false && mouse.last_y != false) {
				...
				// record the track point when it's changed
				if (mouse.x != mouse.last_x || mouse.y != mouse.last_y) {
					points_track.push([mouse.x, mouse.y]);
					console.log("brush moving: x", mouse.x, ", y", mouse.y);
				}
				...
				canvas_active().shadowBlur = 0;
				console.log("points track length", points_track.length);
				console.log("brush released: x", mouse.x + ", y", mouse.y);
			}
			else if (type == 'move') {
				//show size
				canvas_front.clearRect(0, 0, WIDTH, HEIGHT);
	...
	this.sample = function () {
		...
		if (points_sample.length >= POINTS_SAMPLE_MAX) {
			console.log("points sample is enough, clear and try again");
			return;
		}

		if (points_track.length < POINTS_TRACK_NUM_MIN) {
			console.log("points track is too short, try again");
			return;
		}

		// pick the border of track record
		for (i in points_track)	{
			var pt = points_track[i];
			if (rect.left > pt[0])
				rect.left = pt[0];
			if (rect.top > pt[1])
				rect.top = pt[1];
		...
		console.log("track border (left, top, right, bottom) (" 
			+ rect.left + ", " 
			+ rect.top + ", " 
			+ rect.right + ", " 
			+ rect.bottom + ")");
		...
		if (rect.right - rect.left > tmp) {
			tmp = ((rect.right - rect.left) - tmp)/2;
			rect.top -= tmp;
			rect.bottom += tmp;
		}
		else {
			tmp = (tmp - (rect.right - rect.left))/2;
			rect.left -= tmp;
			rect.right += tmp;
		}
		console.log("track border adjusted (left, top, right, bottom) (" 
			+ rect.left + ", " 
			+ rect.top + ", " 
			+ rect.right + ", " 
			+ rect.bottom + ")");

		// pick 10 points from the track array, mark the 
		var x, y;
		var line_w = 10;
		var line_h = 2;		
		var color_rgb = HELPER.hex2rgb('#ff00ff');

		console.log("sample points");
		canvas_active().fillStyle = "rgba(" + color_rgb.r + ", " + color_rgb.g + ", " + color_rgb.b + ", " + ALPHA / 255 + ")";		
		for (i = 0; i < POINTS_SAMPLE_NUM; i++) {
			idx = Math.round(i * points_track.length / (POINTS_SAMPLE_NUM - 1));
			if (idx >= points_track.length)
				idx = points_track.length - 1;

			x = points_track[idx][0];
			y = points_track[idx][1];

			// cross mark for picked point			
			canvas_active().fillRect(x - line_w/2, y, line_w+line_h, line_h);
			canvas_active().fillRect(x, y - line_w/2, line_h, line_w+line_h);
			console.log(idx + ", (" + points_track[idx][0] + ", " + points_track[idx][1] + ")");

			// convert to (0-100) based, and record in sample[]
			x = Math.round((x - rect.left) * 100 / (rect.right - rect.left));
			y = Math.round((y - rect.top) * 100 / (rect.bottom - rect.top));

			y = 100 - y;
			sample.push([x, y]);							
		}

		// convert the points sample to (0 - 100) range for pattern matching
		console.log("sample points converted");
		for (i = 0; i < POINTS_SAMPLE_NUM; i++) {
			console.log(i + ", (" + sample[i][0] + ", " + sample[i][1] + ")");				
		}
		...
		// clear the undo operation
		undo_counter = 0;
	}

	this.reset = function () {
		console.log("reset the records file");
	}

	this.complete = function () {
		console.log("complete and generate sample file, sample length", points_sample.length);
		
		// check and export the generated sample
		var entry = "";
		var length = points_sample.length;
		if (length == 0)
			return;
		
		for (var i = 0; i < length;) {
			for (var j = 0; j < POINTS_SAMPLE_NUM; j++, i++) {
				if (points_sample[i][0]  < 10)
					entry += "  ";
				else if (points_sample[i][0]  < 100)
					entry += " ";

				entry += points_sample[i][0] + ",";

				if (points_sample[i][1] < 10)
					entry += "  ";
				else if (points_sample[i][1] < 100)
					entry += " ";
				entry += points_sample[i][1];				
					
				if (j + 1 == POINTS_SAMPLE_NUM)
					entry += "\r\n";
				else
					entry += ",";
			}
		}
		console.log("sample file detail");
		console.log(entry);
		download_content("sample.log", entry);
	...
	this.undo = function () {
		// clear points track
		console.log("undo the latest step");
		points_track = [];
	...
	function record(sample) {
		for (var i = 0; i < sample.length; i++)
			points_sample.push(sample[i]);

		GUI.draw_sample(sample_pos++, sample);
	}
	
	function download_content(fname, fcontent) {
		var atag = document.createElement("a");
		var file = new Blob([fcontent], {type: 'text/plain',  data: 'application/octet-stream'});
