## from: https://github.com/apachecn/MachineLearning/blob/master/src/python/2.KNN/kNN.py
...
def classify0(inX, dataSet, labels, k):
    ...
    # 1. 距离计算
    dataSetSize = dataSet.shape[0]  # 返回数组的行数
    ...
    # argsort() 是将x中的元素从小到大排列，提取其对应的index（索引)输出。
...
def file2matrix(filename):
    ...
    for line in fr.readlines():
        ...
        # 每列的类别数据，就是 label 标签数据  -1表示最后一个元素
...
def datingClassTest():
    ...
    print("right rate: ", float(numTestVecs - errorCount) / float(numTestVecs))
...
def handwritingClassTest():
    ...
    print("\nthe total right rate is: %f" % ((mTest - errorCount) / float(mTest)))
